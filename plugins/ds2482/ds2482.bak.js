...
class DS2482 {
    ...
    configureBridgeSync(options) {
        ...
        this._i2cWriteSync(commands.WRITE_CONFIG, [((~config & 0x0F) << 4) | config]);

        if (config !== this._readBridgeSync()) {
            throw new Error('Failed to configure bridge');
        }
    }
    ...
    _i2cWriteSync(cmd, bytes) {
            if (bytes) {
                return this.i2c.writeBytesSync(cmd, bytes);
            } else {
                return this.i2c.writeByteSync(cmd);
            }
    }
