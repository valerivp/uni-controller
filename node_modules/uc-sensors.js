...
wscli.commands.add({GetData: String}, (arg)=>{
        if(wscli.context.current === wscli.context.sensor) {
            ...
            let rows = db.querySync("SELECT ID, Type, TimeLabel FROM mem.SensorsData WHERE (ID = $ID OR $ID = 0) AND ReceiveTime > $ReceiveTime", {
...
module.exports.updateSensorData = function(params){
    ...
    q = `DELETE FROM mem.SensorsParams WHERE ID = $ID;
        REPLACE INTO mem.SensorsData (ID, Type, TimeLabel, ReceiveTime) VALUES ($ID, $Type, $TimeLabel, $ReceiveTime);\n`;
