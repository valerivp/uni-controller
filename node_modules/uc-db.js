...
function checkTableSchema(schema, table, data){
    ...
    if(!(data.query || data.trigger || data.view || data.schema || data.data || data['index'] || data['unique index']))
        data = {schema: data};
    ...
    if(data.schema) {
        if (!Object.keys(data.schema).length) {
            console.warn(`Delete table: ${schema}.${table}`);
            db.querySync(`DROP TABLE ${schema}.${table}`);
        } else {
            let sql = buildSqlForCreateTable(schema, data.schema, table, data.constraint);
    ...
    if(data.trigger) {
        db.querySync(`CREATE TRIGGER ${schema === 'main' ? '' : schema + '.'}${table} ${data.trigger}`);
    }
    if(data.query) {
        db.querySync(data.query);
    }
        ...
        for(let name in indexes){
            ...
            if(index.length) {
                ...
                if (index_info && index_info.sql !== sql) {
                    if (schema === 'main') {
                        console.warn(`Recreate index: ${schema}.[${table}.${name}]`);
