...
function recreateTable(schema, table, newTableSchema, newSql) {
    if(schema !== 'main')
    ...
    // Object.getOwnPropertyNames(newTableSchema).forEach(item=>{
    //     newColumns[item.toLowerCase()] = item;
    // });
    ...
    db.querySync(``); // disable foreign key constraint check

    let tempName = `${table}_${Date.now()}`;
    db.querySync(`PRAGMA foreign_keys=off;
        ALTER TABLE ${table} RENAME TO ${tempName};
        ${newSql};` + (columnsList.length ?
        `INSERT INTO ${table}(${columnsList.join(', ')})
            SELECT ${columnsList.join(', ')} FROM ${tempName};` : '') + `
        DROP TABLE ${tempName};
    ...
    if(data.view){
        ...
        if(!Object.keys(table_schema).length){
            ...
            if(!table_info) {
                ...
                recreateTable(schema, table, table_schema, sql);
            }
