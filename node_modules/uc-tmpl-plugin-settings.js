    ...
    wscli.commands.add({SetCount:Number},
        function(arg){
            if(wscli.context.current === wscli.context[pluginName]) {
                ...
                if(count >= arg){
                    let qp = {$Count: arg};
                    if(db.querySync(`SELECT ${PluginName}ID
                        FROM ${PluginNamePN}
                        WHERE ${PluginName}ID > $Count
                            AND Name != '' ORDER BY ${PluginName}ID DESC LIMIT 1`, qp).length){
                        throw new Error(`${PluginName} name not empty`);
                    }

                    let q = `DELETE FROM ${PluginNamePN} WHERE ${PluginName}ID > $Count AND Name = '';
    ...
    function GetInfo(info, arg) {
        if(wscli.context.current === wscli.context[pluginName]){
            ...
            rows.forEach(function (row) { // noinspection JSUnresolvedVariable
                let data = `#${PluginName}:${row.ID},${info}:${wscli.data.toString(row[info])}`;
        ...
        wscli.commands.add({SetName: String},
            function(arg) {
                if(wscli.context.current === wscli.context[pluginName]){
                    ...
                    wscli.sendData(`#${PluginName}:${row.ID},Name:${wscli.data.toString(row.Name)}`);
                    return true;
                }
            },
            `Set ${PluginName} name.`);
        ...
        let tmpl = `{
          ...
          "temp": {
            "${PluginNamePN}InUse":{
                "schema": {
                  "${PluginName}ID": "INTEGER PRIMARY KEY",
                  "OwnerKey": "CHAR(32) NOT NULL"
                }
            }, 
            "${PluginNamePN}CheckInUse":{
                "trigger": "BEFORE UPDATE OF [TypeID] ON ${PluginNamePN}
                    FOR EACH ROW
                    WHEN old.${PluginName}ID in (SELECT ${PluginName}ID FROM temp.${PluginNamePN}InUse)
                    BEGIN
                        SELECT RAISE(ABORT,'Can not change type, ${PluginName} is used');
                    END"
            }  
          }
        }`;

        tmpl = JSON.parse(tmpl.replaceAll('\n', ' '));
        if(dbData)
            utils.mixin(tmpl, JSON.parse(dbData.replaceAll('\n', ' ')));
