...
router.get('/plugins',
    function (req, res) {
        ...
        let q = `SELECT ls.[Name] AS Name, ls.Version AS Version, Directory, CASE WHEN StatusID >= 0 AND p.Name IS NULL THEN 5 ELSE StatusID END AS StatusID, ErrorLog
            FROM mem.PluginsLoadStatus as ls
            LEFT JOIN main.[Plugins] AS p
                ON ls.Name = p.Name
            UNION
            SELECT p.[Name], p.Version, NULL, 4, NULL
                ...
                WHERE s.[Name] IS NULL
            ORDER BY [Name]`;
        res.write(JSON.stringify(db.querySync(` ORDER BY [Name]`)));
        res.end();
    },
    'get plugins info');
