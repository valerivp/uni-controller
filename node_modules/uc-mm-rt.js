...
module.exports.init = function () {
    ...
    let modules = JSON.parse(fs.readFileSync('./www/modules/web-modules.json', 'utf8'));
    Object.keys(modules).forEach(item =>{
        fs.appendFileSync(module.settings.WebJsName, `// ${item}\n`);
        fs.appendFileSync(module.settings.WebJsName, fs.readFileSync(`./www/modules/${modules[item]}`));
    });
...
module.exports.update['0.0.3'] = function(){
    db.init(getDbInitData_0_0_3());
    db.querySync(`UPDATE PluginsSettings SET WebJsName = './www/uc-web-addons.js' WHERE WebJsName = './www/uc-web-plugins.js'`);
};


function getDbInitData_0_0_3() {
    ...
    return `{
          "main": {
            "PluginsSettings": {
              ...
              "data": [
                {"RowID": 1, "WebJsName": "./www/uc-web-addons.js", "CssName": "./www/uc-plugins.css"}
              ]
            },
            "Plugins": {
              "Name": "CHAR(64) NOT NULL PRIMARY KEY",
            ...
            "PluginsStatuses": {
              ...
              "data": [
                ...
                {"StatusID": 4, "Name": "loaded, wait restart"},
                {"StatusID": 5, "Name": "installed, wait restart"}
              ]
            }
          },
          "mem":{
...
router.get('/plugins',
    function (req, res) {
        ...
        let q = `SELECT data.name AS name, version, ps.Name AS status 
        FROM ( 
        SELECT ls.[Name] AS name, ls.Version AS version, directory, CASE WHEN StatusID >= 0 AND p.Name IS NULL THEN 5 ELSE StatusID END AS statusID, errorLog
            FROM mem.PluginsLoadStatus as ls
            LEFT JOIN main.[Plugins] AS p
                ON ls.Name = p.Name
            UNION
            SELECT p.[Name], p.Version, NULL, 4, NULL
                FROM main.[Plugins] AS p
                LEFT JOIN mem.[PluginsLoadStatus] AS s
                    ON p.[Name] = s.Name
                WHERE s.[Name] IS NULL) AS data
            LEFT JOIN main.PluginsStatuses as ps
                ON data.StatusID = ps.StatusID
