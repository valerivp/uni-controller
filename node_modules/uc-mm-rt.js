...
module.exports.loadPlugins = function () {
    ...
    let rows;
    rows = db.querySync(`CREATE TABLE mem.PluginsToLoad AS SELECT * FROM Plugins;
        SELECT DISTINCT p.Name, p.Version AS Version, p.Directory
            FROM mem.PluginsToLoad as p`);
    rows.forEach(row=>{
        let newVersion = false;
        console.state(`Check plugin dependences: '${row.Name}'`);
        ...
        if (rows.length && rows[0].Version !== obj.version)
    ...
    db.querySync(`SELECT * FROM mem.PluginsToLoad`).forEach(row=>{
        let msg = `Plugin ${row.Name} not loaded, because depends on plugin not loaded:`;
        db.querySync(`SELECT DISTINCT p.Name, d.DependsOn
            FROM mem.PluginsToLoad as p
                LEFT JOIN PluginsDependences as d
                    ON p.Name = d.Plugin
                LEFT JOIN mem.LoadedPlugins as l
                    ON d.DependsOn = l.Plugin
            WHERE l.Plugin IS NULL AND p.Name = $Name`, {$Name: row.Name})
            .forEach(row => msg += `\n${row.DependsOn}`);
        console.error(msg);
    });
