...
function checkPluginsDirectory() {
    ...
    db.querySync(`INSERT INTO mem.PluginsLoadStatus(Name, Version, Directory, StatusID)
            SELECT p.[Name], p.Version, p.Directory, 2
...
*/
    ...
    if((obj.web ||{}).js)
        obj.web.js.forEach(function (file) {
            ...
            fs.appendFileSync(settings.WebJsName, fs.readFileSync(`./plugins/${directory}/${file}`));
    ...
    if((obj.web ||{}).css)
        obj.web.css.forEach(function (file) {
            ...
            fs.appendFileSync(settings.CssName, fs.readFileSync(`./plugins/${directory}/${file}`));
        });
...
router.get(urlPlugins,
    function (req, res) {
        ...
        let q = `SELECT lp.Name AS name, p.Version as version
            FROM mem.LoadedPlugins AS lp
            LEFT JOIN Plugins AS p
                ON lp.Name = p.Name
            ORDER BY [Name]`;
